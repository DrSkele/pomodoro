...
fun TaskInputScreen(
    ...
    val workTime =
        rememberInputFieldState(((task?.workTime?.inWholeMinutes?.rem(60)) ?: 25).toString())
    val breakTime =
        rememberInputFieldState(((task?.breakTime?.inWholeMinutes?.rem(60)) ?: 25).toString())
    val longBreak =
        rememberInputFieldState(((task?.longBreakTime?.inWholeMinutes?.rem(60)) ?: 25).toString())

    val breakInterval = rememberInputFieldState((task?.breakInterval ?: 4).toString())
        ...
        task?.copy(
            ...
            longBreakTimeInMillisec =
                longBreak.text.text
                    .toInt()
                    .minutes.inWholeMilliseconds,
            breakInterval = breakInterval.text.text.toInt(),
            dailyGoal = dailyGoal.text.text.toInt(),
            colorNum =
                Color(
                    red.text.text.toInt(),
                    green.text.text.toInt(),
                    blue.text.text.toInt(),
                ).toArgb(),
        ) ?: Task(
            description = name.text.text,
            workTime =
...
class InputFieldState(
    ...
    companion object {
        val Saver: Saver<InputFieldState, Any> =
            mapSaver(
